...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = buildNaturalLanguagePrompt(ingredients, filters, userLanguage)
    ...
    try {
      ...
      recipe = {
        ...
        cookTime: Number(parsedRecipe.cookTime) || 30,
        servings: filters.servings || 2,
        difficulty: ["Easy", "Medium", "Hard"].includes(parsedRecipe.difficulty) ? parsedRecipe.difficulty : "Medium",
        cuisine: parsedRecipe.cuisine || filters.cuisine || "International",
        dietary: Array.isArray(parsedRecipe.dietary) ? parsedRecipe.dietary : filters.dietary ? [filters.dietary] : [],
        image: imageUrl,
...
function buildNaturalLanguagePrompt(userInput: string, filters: RecipeFilters, language: string): string {
  let prompt = `User Request: "${userInput}"\n\n`

  prompt += `IMPORTANT: The user has made a natural language request. Please understand what they want and create an appropriate recipe. They might be:
- Asking for a specific dish by name
- Listing ingredients they have available
- Describing what type of meal they want
- Asking in any language (respond in the same language if possible)

Please interpret their request intelligently and create a recipe that matches their needs.\n\n`
  ...
  if (filters.dietary && filters.dietary !== "any") {
    requirements.push(`Dietary requirement: ${filters.dietary}`)
  }
  ...
  if (filters.spiceLevel && filters.spiceLevel !== "any") {
    requirements.push(`Spice level: ${filters.spiceLevel}`)
  }

  if (filters.mealType && filters.mealType !== "any") {
    requirements.push(`Meal type: ${filters.mealType}`)
  }
  ...
  if (filters.cookTime && filters.cookTime !== "any") {
    ...
    requirements.push(`Cooking time: ${timeMap[filters.cookTime as keyof typeof timeMap] || filters.cookTime}`)
  }

  if (filters.difficulty && filters.difficulty !== "any") {
    requirements.push(`Difficulty level: ${filters.difficulty}`)
  }

  if (filters.healthProfile && filters.healthProfile !== "any") {
    requirements.push(`Health focus: ${filters.healthProfile}`)
  }
...
function calculateAccurateNutrition(parsedRecipe: any, filters: RecipeFilters) {
  ...
  if (filters.dietary === "keto" || filters.dietary === "low-carb") {
    baseCarbs = Math.max(5, baseCarbs * 0.2)
    baseFat = baseFat * 1.8
    baseCalories = baseCalories * 0.9
  } else if (filters.dietary === "high-protein") {
    baseProtein = baseProtein * 1.8
    baseCalories = baseCalories * 1.1
  } else if (filters.dietary === "vegan" || filters.dietary === "vegetarian") {
    baseProtein = baseProtein * 0.8
    baseCarbs = baseCarbs * 1.2
    baseFat = baseFat * 0.9
  } else if (filters.dietary === "low-calorie") {
...
async function createMockRecipeWithImage(userInput: string, filters: RecipeFilters, language: string): Promise<Recipe> {
  ...
  const imageUrl = await generateAccurateRecipeImage(title, description, filters.cuisine)
...
async function createFallbackRecipeWithImage(
  ...
  // Generate image using Gemini
  const imageUrl = await generateAccurateRecipeImage(title, "A delicious recipe", filters.cuisine)
